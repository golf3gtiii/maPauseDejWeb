...
class UploadComponent extends Component {
        ...
        /**
          ...
          *			- rules['type'] = string ('resize','resizemin','resizecrop', 'resizecroptop','crop')
        ...
        function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL, $langue = 'fr') {
                ...
                if (!is_null($allowed)) { $this->_allowed = $allowed; } else { $this->_allowed = array('jpg','jpeg','gif','png', 'PNG'); }
                ...
                if (isset($file) && is_array($file) && !$this->upload_error($file['error'])) {
                        ...
                        if(!in_array($this->ext($fileName),$this->_allowed)){
                                if ($this->langue == 'fr') $this->error("Type de fichier non autoris&eacute;.");
                                else $this->error("File type not allowed.");
                        } else { 
                ...
                //$fileName = str_replace($destination, "", $this->_name);
                //return $fileName;

                //echo $fileName;

                $this->_name = str_replace($destination, "", $this->_name);
        ...
        function image ($file, $type, $size, $output = NULL, $quality = NULL) {
                ...
                switch ($type) {
                        ...
                        case 'resizecroptop':
                                // -- resize to max, then crop to center if x
                                $ratioX = $maxW / $uploadWidth;
                                $ratioY = $maxH / $uploadHeight;
